/**
 * This is a collection of decorators for functions which performs
 * several security checks.They can be combined with each other to
 * configure the necessary constraints for a function that is exposed to the Internet.
...
const switchToHttps = () => {
    ...
    let url = 'https://' + request.httpHost + request.httpPath;
...
const Filters = {
    ...
    /** Action must be accessed via a PUT request */
    put: function () {return request.httpMethod === 'PUT';},
 ...
 * @see module:guard
 */
const ensure = (filters, action, params) => {
...
/**
 * @see module:guard~https
 * @see module:guard~put
 * @deprecated Use ensure(['https','put'], action) instead
 */
exports.httpsPut = function (action) {
    return ensure(['https','put'], action);
};
